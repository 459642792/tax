...
import com.blueteam.base.conf.WxFjjhMapConfig;
...
/**
 * @author xiaojiang
 * @create 2017-08-15  18:15
 */
@DependsOn(value = {"springContextHelper", "userWXLogin"})
@RestController
@RequestMapping("/wechat/login/user")
public class WXUserController {
    private static final HashMap<Integer, UserWXLoginService> WX_LOGINS = new HashMap<>();

    static {
        WX_LOGINS.put(Enums.UserType.Every, (UserWXLoginService) SpringContextHelper.getBean("userWXLogin"));
    }

    @Autowired
    private WeiXinUserService wxMpService;
    ...
    @Autowired
    private WxFjjhMapConfig apiConfig;
    ...
    public BaseResult get(@RequestHeader("User-Agent") String userAgent,
        ...
        if (!StringUtils.isEmpty(code)) {
            System.out.println("codeccodecodecodecodecodecodecodecodeode:" + code);
            ...
            if (valid) {
                ...
                if (WX_LOGINS.containsKey(userType)) {
                    ...
                    token = wxLogin.logIn(wxMpUser, Enums.UserType.Every, Enums.ThirdPartyUserInfo.WEI_XIN, ThirdPartyUserInfo.THIRD_PARTY_STATUS_BIND);
